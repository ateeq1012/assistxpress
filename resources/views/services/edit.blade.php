                        ...
                        <form action="{{ route('services.update', $service->id) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select name="workflow_id" class="form-control" id="workflow_id">
                                    ...
                                    @foreach($workflows as $workflow)
                                        <option value="{{ $workflow->id }}" {{ $service->workflow_id == $workflow->id ? 'selected' : '' }}>{{$workflow->name}}</option>
                            ...
                            <!-- <div class="form-group">
                                ...
                                <select name="is_planned" class="form-control" id="is_planned">
                                    ...
                                    <option value="1" {{ $service->is_planned == 1 ? 'selected' : '' }} >Yes</option>
                                    <option value="0" {{ $service->is_planned == 0 ? 'selected' : '' }} >No</option>
                                </select>
                            </div> -->

                            <div class="form-group">
                                <label for="color">Color :</label>
                                <input type="color" class="form-control p-0" name='color' id="color" value="{{ $service->color }}"  title="Choose your color">
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="enabled" class="form-control" id="enabled">
                                    <option value="">Select an option</option>
                                    <option value="1" {{ $service->enabled ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ $service->enabled ? '' : 'selected' }}>No</option>
                                </select>
                            </div>
    ...
    function save_fields() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '{{ route("services.save_service_custom_fields") }}',
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        id: @json($service->id),
