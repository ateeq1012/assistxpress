                    ...
                    <div class="tab-content">
                        <div role="tabpanel" id="tab-filters" class="tab-pane active">
                            <div class="panel-body pb-1">
                                <div class="row">
                                    <div class="form-group pl-1 pr-1" style="width: 160px;">
                                        <input type="number" placeholder="Service Request ID" id="id" class="form-control srch_col">
                                    </div>
                                    ...
                                    <div class="form-group pl-1 pr-1" style="min-width: 200px;">
                                        <select id="service_domain_id" class="form-control srch_col select2-field" data-placeholder="Service Domain">
                                            <option value="">Service Domain</option>
                                            @foreach ($service_domains as $service_domain)
                                                <option value="{{$service_domain['id']}}">{{$service_domain['name']}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group pl-1 pr-1" style="min-width: 200px;">
                                        <select id="service_id" class="form-control srch_col select2-field" data-placeholder="Service">
                                    ...
                                    <div class="form-group pl-1 pr-1" style="min-width: 200px;">
                                        <select id="sla_rule_id" class="form-control srch_col select2-field" data-placeholder="SLA Rule">
                                            <option value="">SLA Rule</option>
                                            @foreach ($sla_rules as $sla_rule_id => $sla_rule_name)
                                                <option value="{{$sla_rule_id}}" >{{$sla_rule_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group pl-1 pr-1" style="min-width: 200px;">
                                ...
                                <div class="row pl-1">
                                    <div class="form-group mb-0">
                                        ...
                                        @if(session('user_routes')['service_requests.download'] ?? false)
                                            <form action="{{ route('service_requests.download') }}" method="POST" style="display: inline-block;" target="_blank" id="download-form">
                                                @csrf
        ...
        tsk_dt = $('#tsk_tbl').DataTable({
            ...
            "ajax": function (data, callback, settings) {
                ...
                if (val_changed) {
                    ...
                    $.ajax({
                        url: "{{ route('service_requests.get_service_request_data') }}",
                        type: 'POST',
                        data: {
                            draw: data.draw,
                            start: data.start,
            ...
            "columnDefs": [
                ...
                },
                {
                    ...
                    "render": function (data, type, row, meta) {
                        return `
                            @if(session('user_routes')['service_requests.show'] ?? false)
                                <a href="/service_requests/${row.id}" class="btn btn-info btn-xs">View</a>
                            @else
                                <a class="btn btn-default btn-xs permission-denied">View</a>
                            @endif
                            @if(session('user_routes')['service_requests.edit'] ?? false)
                                <a href="/service_requests/${row.id}/edit" class="btn btn-primary btn-xs">Edit</a>
                            @else
                                <a class="btn btn-default btn-xs permission-denied">Edit</a>
                            @endif
                            @if(session('user_routes')['service_requests.destroy'] ?? false)
                                <form action="/service_requests/${row.id}" method="POST" style="display: inline-block;">
