...
<div class="ibox pt-2">
    <div class="ibox-title">
        ...
        <div class="ibox-tools">
            ...
            @if(session('user_routes')['service_domains.add_users_bulk'] ?? false)
        ...
        <div class="row">
            <div class="col-6">
                <div class="panel panel-primary">
                    ...
                    <div class="panel-body" style="max-height: 750px; overflow: auto;">
                        <form action="{{ route('service_domains.update', $service_domain->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="name">Service Domain Name</label>
                                <input type="text" name="name" class="form-control" value="{{ $service_domain->name }}" required>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="enabled" class="form-control" id="enabled">
                                    ...
                                    <option value="1" {{ $service_domain->enabled ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ $service_domain->enabled ? '' : 'selected' }}>No</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="closed">Service Domain Closed</label>
                                <select name="closed" class="form-control" id="closed">
                                    <option value="">Select an option</option>
                                    <option value="1" {{ $service_domain->closed ? 'selected' : '' }}>Yes</option>
                                    <option value="0" {{ $service_domain->closed ? '' : 'selected' }}>No</option>
                                </select>
                            </div>

                            <div class="form-group">
                                <label for="color">Color</label>
                                <input type="color" class="form-control p-0" name='color' id="color" value="{{ $service_domain->color }}"  title="Choose your color">
                            </div>
                            <div class="form-group">
                                ...
                                @foreach ($services as $service)
                                    <div class="form-check">
                                        <input 
                                            ...
                                            {{ !empty(old()) ? (old('services') !== null && array_key_exists($service['id'], old('services')) ? 'checked' : '') : ($service['checked'] ? 'checked' : '') }}>
            ...
            <div class="col-6">
                <div class="panel panel-info" id="group-members-box">
                    ...
                    <div class="panel-body" style="max-height: 750px; overflow: auto;">
                        @if(session('user_routes')['service_domains.add_groups'] ?? false)
                        <form action="{{ route('service_domains.add_groups', $service_domain->id) }}" method="POST">
                                ...
                                <div class="d-inline-block mr-2">
                                    @if(session('user_routes')['groups.show'] ?? false)
                                        <a href="{{ route('groups.show', $gr->id) }}" target="_blank" class="btn btn-default">
                                            ...
                                            @if(session('user_routes')['service_domains.remove_group'] ?? false)
                                                <form action="{{ route('service_domains.remove_group', [$service_domain->id, $gr->id]) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="panel panel-success" id="group-members-box">
                    ...
                    <div class="panel-body" style="max-height: 750px; overflow: auto;">
                        ...
                        @if(session('user_routes')['service_domains.add_users'] ?? false)
                        <form action="{{ route('service_domains.add_users', $service_domain->id) }}" method="POST">
                            <div id="add-user-to-group-form">
                                @csrf
                                <div class="form-group">
                            ...
                            <table class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th> Phone Number </th>
                                        <th> Role </th>
                                        @if(session('user_routes')['service_domains.remove_user'] ?? false)
                                        <th> Actions </th>
                                        @endif
                                    </tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @if(session('user_routes')['service_domains.remove_user'] ?? false)
                                        <td>
                                            <form action="{{ route('service_domains.remove_user', [$service_domain->id, $row->id]) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Bulk Add Users to Service Domain</h4>
            </div>
            <div class="modal-body">
                ...
                <form action="{{ route('service_domains.add_users_bulk', $service_domain->id) }}" method="POST">
                    @csrf
                    <textarea name="emails" class="form-control" rows="10" placeholder="email1&#10;email2&#10;.&#10;.&#10;."></textarea>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-white" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-primary">Add Users to Service Domain</button>
                    </div>
                </form>
            </div>
        </div>
