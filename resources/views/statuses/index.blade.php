...
<div class="ibox pt-2 container">
    ...
    <div class="ibox-content">
        ...
        <form method="GET" action="{{ route('statuses.index') }}" class="form-inline mb-2">
        ...
        <table class="table table-striped table-bordered mt-3">
            <thead>
                <tr>
                    <th><a href="{{ route('statuses.index', array_merge(request()->all(), ['sort' => 'id', 'direction' => $sortDirection === 'asc' ? 'desc' : 'asc'])) }}">ID</a></th>
                    ...
                    <th><a href="{{ route('statuses.index', array_merge(request()->all(), ['sort' => 'updated_at', 'direction' => $sortDirection === 'asc' ? 'desc' : 'asc'])) }}">Updated on</a></th>
            ...
            <tbody>
                @foreach ($statuses as $status)
                    <tr>
                        ...
                        <td>
                            @if(session('user_routes')['statuses.show'] ?? false)
                                <!-- <a href="{{ route('statuses.show', $status->id) }}" class="btn btn-info btn-xs">View</a> -->
                            @else
                                <!-- <a href="#" class="btn btn-info btn-xs disabled">View</a> -->
                            @endif
                            @if(session('user_routes')['statuses.edit'] ?? false)
                                <a href="{{ route('statuses.edit', $status->id) }}" class="btn btn-primary btn-xs">Edit</a>
                            @else
                                <a href="#" class="btn btn-primary btn-xs disabled">Edit</a>
                            @endif
                            @if(session('user_routes')['statuses.destroy'] ?? false)
        ...
        showing {{ $statuses->count() }} of {{ $statuses->total() }} records {{ $statuses->appends(['search' => request('search'), 'page_size' => $pageSize, 'sort' => $sortColumn, 'direction' => $sortDirection])->links('pagination::bootstrap-4') }}
    </div>
</div>
