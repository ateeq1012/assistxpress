...
class SlaHelper
{
    public static function getSlaInfo($task_status, $task, $sla_rule)
    {
        ...
        if( isset($task->sla_rule_id) )
        {
            ...
            $response_time_spent_sec = $task->tto ?? 0;
            $resolution_time_sla_sec = GeneralHelper::hh_mm_to_sec($resolution_time);
            $resolution_time_spent_sec = $task->ttr ?? 0;

            $response_time_sla = $response_time;
            $response_time_spent = isset($task->tto) ? GeneralHelper::sec_to_hh_mm($task->tto) : '';

            $response_sla_percentage = ($response_time_sla_sec > 0) ? round(($task->tto / $response_time_sla_sec) * 100 , 2) : 0;
            
            $response_sla_status = null;
            if($response_sla_percentage > 100) {
                $response_sla_status = 'SLA Breach';
            } else if(isset($task->response_time) ) {
            ...
            $resolution_time_spent = isset($task->ttr) ? GeneralHelper::sec_to_hh_mm($task->ttr) : '';
            $resolution_sla_percentage = ($resolution_time_sla_sec > 0) ? round(($task->ttr / $resolution_time_sla_sec) * 100 , 2) : 0;
            
            $resolution_sla_status = null;
            if($resolution_sla_percentage > 100) {
                $resolution_sla_status = 'SLA Breach';
            } else if(in_array($task_status['type'], [3,4]) ) {
